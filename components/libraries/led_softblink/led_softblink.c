...
ret_code_t led_softblink_init(led_sb_init_params_t const * p_init_params)
{
	...
	nrf_gpio_cfg_output(LED_PWM_RED_PIN);
	nrf_gpio_cfg_output(LED_PWM_GREEN_PIN);
	nrf_gpio_cfg_output(LED_PWM_BLUE_PIN);
	...
	nrf_gpio_pin_set(LED_PWM_RED_PIN);
	nrf_gpio_pin_set(LED_PWM_GREEN_PIN);
	nrf_gpio_pin_set(LED_PWM_BLUE_PIN);

	// Is initialization code running on zeroed struct?
    ASSERT(m_led_sb.led_sb_state == NRF_DRV_STATE_UNINITIALIZED);
	// Is init params non-null?
    ASSERT(p_init_params);

	// Are init params members sane?
...
//    APP_TIMER_DEF(led_softblink_timer);
	// TODO untested
	static app_timer_t led_softblink_timer_data = { {0} }; // unused except in following line.
    static const app_timer_id_t led_softblink_timer = &led_softblink_timer_data;
	...
	// Start pwm driver with 2nd smallest possible duty cycle?
